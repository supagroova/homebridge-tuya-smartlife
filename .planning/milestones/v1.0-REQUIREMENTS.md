...
- [x] **AUTH-01**: User completes Smart Life login via User Code + QR scan (no developer account)
- [x] **AUTH-02**: Plugin authenticates through the Smart Life QR device-sharing path using the Tuya-published HA-compatible `client_id`/`schema` for development; Homebridge-specific credentials remain a release-hardening follow-up
- [x] **AUTH-03**: Auth tokens persist across Homebridge restarts (no QR re-scan on reboot)
- [x] **AUTH-04**: Tokens auto-refresh proactively before expiry; clear re-auth surfaced when refresh fails
- [x] **AUTH-05**: Signed-request transport (HMAC + AES-GCM `encdata`) ported correctly, locked by golden-vector tests

### Device Discovery

- [x] **DISC-01**: Plugin discovers the user's Tuya devices from the cloud (homes → devices → category + status set)
- [x] **DISC-02**: Accessories use stable UUIDs (from Tuya device id); restored from cache without duplicates
- [x] **DISC-03**: Devices removed from the Tuya account are pruned from HomeKit
- [x] **DISC-04**: Optional home/device whitelist via config

### Switches & Outlets

- [x] **SW-01**: User can turn switches/outlets on and off from HomeKit
- [x] **SW-02**: Multi-gang devices expose each gang as its own HomeKit service
- [x] **SW-03**: Outlets vs plain switches are mapped to the appropriate HomeKit service type

### Climate & Sensors

- [x] **CLIM-01**: User can view temperature/humidity sensor readings in HomeKit (correctly scaled)
- [x] **CLIM-02**: Contact, motion, leak, and smoke sensors report state in HomeKit
- [x] **CLIM-03**: User can read and set thermostat state (target temp within DP-spec range/step; mode)
- [x] **CLIM-04**: Battery level / low-battery reported where the device provides it

### Status & Reliability

- [x] **UPD-01**: Device state stays current via polling (configurable interval, with jitter/backoff)
- [x] **UPD-02**: `onGet` returns cached state immediately and never blocks on the network
- [x] **UPD-03**: Offline devices report "No Response" in HomeKit rather than stale/false state
...
| SW-01 | Phase 4 | Complete |
| SW-02 | Phase 4 | Complete |
| SW-03 | Phase 4 | Complete |
| CLIM-01 | Phase 5 | Complete |
| CLIM-02 | Phase 5 | Complete |
| CLIM-03 | Phase 5 | Complete |
| CLIM-04 | Phase 5 | Complete |
| UPD-01 | Phase 6 | Complete |
| UPD-02 | Phase 6 | Complete |
| UPD-03 | Phase 6 | Complete |
...
*Archived: 2026-07-01 after v1.0 release completion*
