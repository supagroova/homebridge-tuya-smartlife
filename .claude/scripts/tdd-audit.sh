...
#
# A path listed in .claude/tdd-debt.txt is reported as non-fatal debt; any other
...
if [[ ${#debt_remaining[@]} -gt 0 ]]; then
  echo "TDD audit OK — but ${#debt_remaining[@]} production file(s) remain in tdd-debt.txt:" >&2
  for f in "${debt_remaining[@]}"; do echo "  ! $f" >&2; done
  echo "Address these over time; remove each line from .claude/tdd-debt.txt as its test ships." >&2
fi
